...
def load_genotype_data(file_path):
    """加载基因型数据"""
    print(f"Loading genotype data: {file_path}")
    genotype_data = pd.read_csv(file_path, sep='\t', dtype=str, low_memory=False)
    first_col_values = genotype_data.iloc[:, 0].tolist()
    first_value = str(first_col_values[0]) if first_col_values else ""
    if '/' in first_value:
        hybrid_ids = genotype_data.iloc[:, 0].tolist()
        genotype_values = genotype_data.iloc[:, 1:].apply(pd.to_numeric, errors='coerce').fillna(0.5).values.astype(np.float32)
    else:
    ...
    print(f"  Loaded {len(hybrid_ids)} genotypes, {genotype_values.shape[0]} SNPs (shape: {genotype_values.shape})")
...
def load_environment_data(file_path):
    """加载环境数据"""
    print(f"Loading environment data: {file_path}")
...
def prepare_dataset(genotype_path, env_path, pheno_path, test_path=None):
    ...
    print(f"Loading phenotype data: {pheno_path}")
    ...
    print(f"Original phenotype rows: {len(pheno_data)}")
    pheno_data = pheno_data.drop_duplicates(subset=['Hybrid', 'Environment'], keep='first')
    print(f"After deduplication: {len(pheno_data)}")
    ...
    # 打印每个性状的有效数据量
    for trait_key, col_name in trait_col_map.items():
        print(f"{trait_key} ({col_name}) valid data: {len(trait_data[trait_key])} samples")
        if test_path and trait_key in test_data_dict:
            print(f"{trait_key} ({col_name}) test data: {len(test_data_dict[trait_key])} samples")
